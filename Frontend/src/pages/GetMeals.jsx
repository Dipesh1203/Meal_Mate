    ...
    const fetchDonations = async () => {
      try {
        const response = await axios.get(
          "https://beta-8-virid.vercel.app/provider_list/get/donation-meal/all"
        );
    ...
    if (quantity && quantity > 0) {
      try {
        const response = await fetch(
          "https://beta-8-virid.vercel.app/reciver/donation-center/reserve_meal",
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              donation_meal_id: donationId,
              quantity: quantity,
            }),
          }
        );
        ...
        if (response.ok) {
          toast.success(
            `Meal reserved: ${quantity} meals from Donation ID: ${donationId}`
          );
        } else {
          toast.error(
            data.message || "Failed to reserve meal. Please try again."
          );
        }
      } catch (error) {
        ...
        >
          ...
          <tbody>
            {donations.map((donation) => (
              <tr
                key={donation.donation_meal_id}
                className="border-t hover:bg-teal-100"
              >
                <td className="px-4 py-2 text-gray-700">
                  {donation.donation_meal_id}
                </td>
                <td className="px-4 py-2 text-gray-700">
                ...
                <td className="px-4 py-2 text-center">
                  <FmdGoodIcon
                    ...
                    onClick={() =>
                      handleMapClick(donation.latitude, donation.longitude)
                    }
                  />
                </td>
              </tr>
